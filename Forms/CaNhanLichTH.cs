...
{
    public partial class CaNhanLichTH : Form
    {
        public CaNhanLichTH()
        {
            InitializeComponent();
        ...
        private void CaNhanLichTH_Load(object sender, EventArgs e)
        {
        ...
        // Phương thức để lấy MaNV từ bảng NhanVien dựa trên Accountid
        ...
        private void load_datagrid()
        {
            ...
            string sql = "select lth.mastt, pm.tenpm, nv.tennv, lop.tenlop, mon.tenmon, ca.tenca, lth.thu, lth.ngaybd, lth.ngaykt,lth.mapm,lth.magv,lth.malop,lth.mamon,lth.maca from tbllich lth JOIN tblphongmay pm on lth.mapm = pm.mapm " +
                ...
                "join tblcahoc ca on lth.maca = ca.maca where magv = '"+maNV+"'";
        ...
        private void btnluu_Click(object sender, EventArgs e)
        {
            ...
            //check giáo viên
            string giaovienlich = "SELECT * FROM tbllich WHERE (magv = '" + txtGiaovien.Text + "') AND (thu = N'" + cbothu.Text + "') AND (maca = '" + cboca.SelectedValue.ToString() + "')AND NOT ((ngaybd > '" + Class.Function.convertdatetime(mskngayketthuc.Text) + "') OR (ngaykt < '" + Class.Function.convertdatetime(mskngaybatdau.Text) + "'))  ";
            if (Class.Function.CheckKey(giaovienlich))
            {
                MessageBox.Show("Bạn đã có lớp học khác vào thời gian này!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            string sql = "insert into tbllich(mastt,mapm,magv,malop,mamon,maca,thu, ngaybd,ngaykt)values('" + txtmastt.Text + "','" + cbophongmay.SelectedValue.ToString() + "','" + txtGiaovien.Text+ "','" + cbolop.SelectedValue.ToString() + "','" + cbomon.SelectedValue.ToString() + "','" + cboca.SelectedValue.ToString() + "',N'" + cbothu.Text + "', '" + Class.Function.convertdatetime(mskngaybatdau.Text) + "','" + Class.Function.convertdatetime(mskngayketthuc.Text) + "')";
        ...
        private void btnsua_Click(object sender, EventArgs e)
        {
            ...
            //check phòng học đã có lớp chưa
            string phong = "select * from tbllich where (mapm = '" + cbophongmay.SelectedValue.ToString() + "') and (maca ='" + cboca.SelectedValue.ToString() + "') and not (magv='"+txtGiaovien.Text+"') and NOT ((ngaybd > '" + Class.Function.convertdatetime(mskngayketthuc.Text) + "') OR (ngaykt < '" + Class.Function.convertdatetime(mskngaybatdau.Text) + "')) and (thu = N'" + cbothu.Text + "')";
            if (Class.Function.CheckKey(phong))
            {
                MessageBox.Show("Phòng máy đã có lớp đăng ký, bạn cần chọn phòng máy khác!!!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            ...
            //check giáo viên
            string giaovienlich = "SELECT * FROM tbllich WHERE (magv = '" + txtGiaovien.Text + "') AND (thu = N'" + cbothu.Text + "') AND (maca = '" + cboca.SelectedValue.ToString() + "') AND NOT ((ngaybd > '" + Class.Function.convertdatetime(mskngayketthuc.Text) + "') OR (ngaykt < '" + Class.Function.convertdatetime(mskngaybatdau.Text) + "'))  AND mastt != '" + txtmastt.Text + "'";
            if (Class.Function.CheckKey(giaovienlich))
            {
                MessageBox.Show("Bạn đã có lớp học khác vào thời gian này!", "Thông báo", MessageBoxButtons.OK, MessageBoxIcon.Error);
                return;
            }
            string sql = "update tbllich set mapm = '" + cbophongmay.SelectedValue.ToString() + "', malop = '" + cbolop.SelectedValue.ToString() + "', mamon = '" + cbomon.SelectedValue.ToString() + "',maca = '" + cboca.SelectedValue.ToString() + "',thu = N'" + cbothu.Text + "', ngaybd = '" + Class.Function.convertdatetime(mskngaybatdau.Text) + "', ngaykt = '" + Class.Function.convertdatetime(mskngayketthuc.Text) + "' where mastt ='" + txtmastt.Text + "' ";
            Class.Function.RunSql(sql);
            load_datagrid();
            resetvalue();
            btnthem.Enabled = true;
